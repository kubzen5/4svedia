...
# Public ngrok Tunnel and Verification (Windows)

1. Create a free account at [https://dashboard.ngrok.com/signup](https://dashboard.ngrok.com/signup), then copy your authtoken from [https://dashboard.ngrok.com/get-started/your-authtoken](https://dashboard.ngrok.com/get-started/your-authtoken).

2. Install ngrok from the Microsoft Store or use the command shown at [https://ngrok.com/download/windows](https://ngrok.com/download/windows). Verify the installation:

   ```powershell
   ngrok help
   ```

3. Connect the agent to your account. You only need to do this once:

   ```powershell
   ngrok config add-authtoken "PASTE_YOUR_AUTHTOKEN_HERE"
   ```

4. Start the proxy in the first terminal:
...
5. Expose port `3000` in a second terminal:

   ```powershell
   ngrok http 3000
   ```

6. Keep both terminals open. In a third terminal, verify and submit the endpoint:
...
The script automatically retrieves the public HTTPS address from the ngrok agent API on port `4040`, checks the public `/health` endpoint, appends `/assistant`, and submits the resulting URL to Agent Hub as a `proxy` task with a random `sessionID`.

The verifier’s response, including any flag, will appear in the terminal.

Optionally, you can add the full endpoint URL to `PROXY_PUBLIC_URL` in the `.env` file to skip automatic detection.

If your browser displays `ERR_NGROK_6024`, this is ngrok’s one-time protection page for free tunnels. Click the continue button or test the API with the following header:
...
The `s01e03_verify.py` script adds this header automatically.
# S04E04 — filesystem
